        ...
        for message in st.session_state.messages:
            with st.chat_message(message["role"], avatar=getUserIcon(message["role"])):
                ...
                if "btn_type" in message:
                    print(st.session_state.query_running)
        ...
        with st.chat_message("assistant", avatar=getUserIcon("assistant")):
            ...
            for source in sources:
                ...
                with st.expander("**Source:** " + source['file'].replace("target-dir\\","")):
                    ...
                    btn_key = st.session_state.global_btn_key
                    
                    if source['file'] != 'FAQ_file':
                        create_button(btn_key,source['file'])
                        st.session_state.messages.append({"role": "assistant", "content": source_message, "btn_type": "general", "btn_key": btn_key, "source_file": source['file']})
                        
                    else:
                        create_faq_button(btn_key)
                        st.session_state.messages.append({"role": "assistant", "content": source_message, "btn_type": "faq", "btn_key": btn_key, "source_file": source['file']})
